...
async function initializeSchema(db, appInstance = null) {
  ...
  // =========================================================================
  // Session Metadata Table (Two-Table Architecture)
  //
  // Edge Case #3: Enables instant session listing without loading analysis data.
  // All sessions visible immediately; analysis is lazy-loaded separately.
  // =========================================================================
  db.exec(`
  ...
  try {
    ...
    safeLog.log('Created session_metadata table, indexes, and triggers');
  } catch (metadataTriggerError) {
    safeLog.warn('Session metadata trigger creation warning:', metadataTriggerError.message);
  }
